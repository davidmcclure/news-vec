...
Series of recent studies of polarization and fragmentation in media landscape. Filter bubbles / echo chambers; more recently, studies of media consumption patterns via browsing history, cross linking patterns on political blogs, and sharing behavior on Twitter.

Most of these studies take as their jumping off point a representation of the media ecosystem as a social graph, a network of media sources that are bound together based on the similarities in the audiences that read / share content. Fundamental unit of analysis is often the reader, user, viewer, listener -- for Zuckerman, users sharing content on Twitter; for XX, people reading articles on web via search, social referral, etc. By tracking who reads what, possible to model a matrix of similarities among outlets, that capture level of audience overlap or correlation.

But of course, part and parcel with this question of difference at the level of audience is the question of difference at the level of the raw content that is getting produced by different outlets. Which of course, plays as much of a role in processes of polarization and fragmentation as do differences at the level audience - if all news organizations produced identical content, then audience fragmentation wouldn't matter. Concerns about filter bubbles / echo chambers are fundamentally about an interaction between the audience graph and content graph -- specifically, when the become both highly fragmented and also highly aligned -- when isolated audiences are being exposed to isolated content.

Interestingly, though, less is known about the empirical structure of the content graph than the audience graph. Just how different, in a very precise sense, is Fox from NYT, Breitbart from The Hill, WSJ from Bloomberg? This is the sort of question about which there is a profusion of assumption / expectation / common wisdom, but little data. In a number of studies, similarities and differences at the level of content are just directly derived in terms of the audience of the outlet -- eg, label outlets left/right based on the % of audience who follow trump, voted for a republican candidate, etc. The content of an outlet is inferred, essentially, from the composition of its audience. Which assumes, if only implicitly, that content and audience are essentially one-in-the-same.

Some studies have gone beyond this, and started to systematically model the content of news organizations directly and independently of audience. Most notable is work emerging from media cloud -- topic models of articles, which makes it possible to see broad strokes of how different outlets focus on different issues; or, recent work from Zuckerman on media dynamics during 2016 election, which explores the degree to which different outlets (and different groups of outlets) focused attention on different issues.

For the most part, though, this work has taken place separate from -- or in a sense, subsequent to -- the modeling of the media ecosystem as a social graph. The social graph often represents a kind of ground truth, a point of reference against which differences in content are situated. And, for the most part, when these differences have been studied they have been deployed in the context of interpretation; not formulated as a set of measurements that can be put back into direct conversation with other quantitative views of the media ecosystem.

What's missing, then, is a systematic and precise model of the content graph. In a very simple sense -- how different, precisely, are different outlets from each other, purely in terms of the content that they produce? Beyond just characterizing differences at the level of content, can we quantify them with a level of rigor and systematicity that would match the precision with which we can model the audience level affinities among outlets?
...
In other words -- by quantifying similarities at the level of content in precise ways, we can then contextualize those differences in meaningful and actionable ways. Where, in essence, are outlets more or less similar than we would expect -- given the composition of the underlying audiences, and the historical structure of the media landscape from last week, last month, last year?

This thesis, then, explores three questions:

1. How different, precisely, is the content produced by different media organizations?

1. How do (or don't) these similarities at the level of content align with similarities at the level of audience? Does similarly of content correlate with similarity of audience? Where, if ever, does this break down? Are there examples of outlets that speak in a way that's mismatched with the audience - where audiences are being exposed to issues, styles, viewpoints that they otherwise would be unlikely to encounter?

1. Separate from the regular churn of news coverage from day to day - (how) have outlets changed over time in terms of the type of coverage they produce? What has changed, what's changing?

So, as a first step -- how to measure the difference between two news outlets, purely in terms of the content they produce? We need a kind of computational-linguistic measuring stick. What exactly should this look like?
