...
This question of the \textit{distinguishability} or \textit{separability} of headlines touches on question that sits at the core of a number of recent studies that have explored processes of fragmentation and polarization in the media ecosystem. Building generally on the widely-popularized notions of "filter bubbles" and "echo chamgers" developed Parisier and Sunstein, a rich line of work has started to document the degree to which new patterns of news production and consumption have pushed audiences into more isolated spaces where they're less likely to encounter counter-attitudinal views. Flaxman et al. analyzed a large dataset of web browsing histories (volunteered by users of a browser extension) and explored the degree to which different patterns of content discovery push readers towards more or less ideologically polarized content, finding interestingly mixed results -- social media and search results expose users to more polarized content, but also expose them to a wider range of views. Meraz studied the structure of hyperlinks among a set of political blogs during the 2012 presidental election, finding that left-leaning sources were more densely connected than right-leaning sources. And, most recently, Benkler et al. explored the outsized influence of right-leaning blogs -- particularly Breitbart -- on media coverage during the 2016 presidental election, building on a map of the media ecosystem derived from correlations in sharing patterns on Twitter and Facebook (the same technique used to model audience-level similarity later in this project).

- in many cases, these studies have taken as their jumping-off point these types of relationships among outlets at the level of audience composition -- overlap at the level of readers, viewers, listeners. to reason about the implications of these audience-level trends, though, have to map againt salient difference in content, which plays at least as much of a role in processes of fragmentation and polarization as does the question of audience distance -- if all outlets produced identical content, then audience fragmentation wouldn't matter. concerns about filter bubbles / echo chambers are fundamentally about an interaction between the "audience graph" and "content graph" -- specifically, when they become both highly fragmented and also highly aligned, when isolated audiences are being exposed to isolated content.

- how to operationalize these differences at the level of conent? in a number of cases, content-level similarities have been derived directly from audience composition -- eg, \% of audience who follows trump on twitter, voted republican in last election; which assumes that differences at the level of content are strongly correlated with differences in audience; differences in content are literally defined in terms of the audiences. recently, some work has gone beyond this, most notably in the context of the media cloud project at the Center for Civic Media at the MIT Media Lab -- chuang et al with topic models in articles, Benkler with analysis of issues during 2016 election. meanwhile, at a smaller scale, long tradition of content analysis in media studies / polisci that has studied the representation of specific issues in media, though generally the focus is on these issues themselves, and explored via comparatively small datasets.

- generally, though, less is known, empirically about content graph than audience graph. or, at least -- our knowledge about content graph is less precise. one reason for this, perhaps, is that measuring textual distance is fundamentally more complex and open-ended than measuring audience similarity / overlap, where a literal multiple-membership structure that makes it possible to measure distance just by counting things, essentially --- number of shared users, number of times the same user tweets articles to different outlets, etc. whereas with text, Underwood notes, the question how scoring the similarity between two pieces of text is a strand that unravels into a large and complex set of questions. (could be argued that a sizable portion of nascent field of CA, offshoot of DH focused on text analysis, is concerned with some formulation of this question.)

- if we approach this head-on, though -- what could we learn from a very precise representation of the content graph? beyond just describing differences in an exploratory, unsupervised setting, can we quantify them with a level of rigor and systematicity that would match the precision with with we can model audience level similarities among outlets? how to *measure* them in a way that would make it possible to put them back into conversation with other quantitative views of the media ecosystem?

- this is fundamentally the question that this thesis takes up. My conjecture is that there are a number of intersting (but difficult) questions that become tractable when we're able to precisely quantify these content-level similarities. First, if we can say, for example that outlets A and B are 1.0 apart, and B and C are, say, 1.6 apart, then this makes it possible to directly compare the content graph to the underlying audience graph in a very fine-grained, high-resolution way. Which, in turn, opens the door to empirical study of the fundamental assumptions around notions of polarization and fragmentation. Instead of simply assuming that content and audience move in lockstep -- how true is this actually? And, maybe most interesting -- when, if ever, is this not true? Are there cases of misalignment, where, for example, two outlets produce similar content, but have highly divergent audiences that otherwise occupy different ideological / cultural regions? If these exist, could be thought of as naturally-occurring bridges that work against fragmentation; which, in turn, could he highly interesting in the context of efforts to design content / patterns of distribution that work against processes of fragmentation.
...
to get a snapshot of the content produced by different news outlets, we could just scrape their websites directly. but, it's difficult to then consisently map this text data back to fine-grained patterns of audience behavior -- just from the raw web pages, no way to know who read them, when, etc. and, even if we had access to some approximation of this from internal data collected by the news organization, it would then be difficult, if not impossible, to link across multiple outlets and get a holistic view of how the same readers engage with multiple sources.
this is where data from centralized services becomes very valuable -- from platforms like facebook, twitter, and reddit, we can get information about uniquely identified users interacting with content from a range of sources in the context of a relatively controlled setting. eg, flaxman et al. used browser history volunteered by users of the Bing Search Bar, an extension for IE. here, we follow Benkler et al. in using *links to news articles shared on twitter* as the fundamental unit of data.\footnote{of course, we have no interest here in characerizing the behavior of specific users -- interested in behavior at the scale of the individual user, but only in an aggregate sense, spread across tens of millions of accounts. this study never reports any text data or sharing activity that could be linked back to individual people.}
...
this, in a sense, gives us precisely what we need -- a representation of content (headlines), directly linked to a representation of audience (twitter users, tweets)

what are the impliations of using twitter + headlines though?
as for twitter -- downside of course is that twitter, in very well-documented ways, isn't representative of the full media ecosystem; it's one specific (and in many ways idiosyncratic) social network. but, the advantage is that we can explore these questions with a level of scale and, perhaps even more important, consistency that would otherwise be impossible. twitter prevents us from making "wide" claims about the media ecosystem -- we can at most make "platform-studies" claims about what happens on twitter. but, with this loss of generality, we also gain the ability to explore these questions with a level of depth and specificity that would otherwise likely be impossible.

as for headlines -- the downside, clearly, is that we don't get a view of the full content in the underlying articles; and, relationship between hls and article bodies is somewhat unclear, meaning that we can't use the headlines as a proxy for the article content, a generalization about the full content profile of an outlet.
but, at the same time -- headlines themselves are incredibly interesting textual objects -- an elusive mix of summary, advertisement, quotation, metonymy, provocation; and, beyond the linguistic and (and even para-literary) interest of hls, there are a number of more intellectually pragmatic advantages to focusing on hls in this context
...
second, far from being a kind of afterthought, there's a (perhaps distressing) way in which headlines are themselves the substance of the news -- according to a 2014 study from the american press institute, just ~41\% of readers say that they "go beyond the headlines" to dive deeper into stories of interest.
hls represent extraordinarily high-leverage langauge. if we imagine placing different types of textual objects on a continuum that represents the level of social, political, economic "stopping power" -- then hls, on a per-word basis, would sit very near (if not at) the top of this list
if interested in the sites at which language *acts* on the world in manifest ways -- headlines are hard to beat.

also have the advantage of building on a rich line of work in (computational) linguistics on headlines. at a more theoretical register, Dor describes headlines in the context of linguistic notions of "relevance optimization" -- they try to maximize contextual effects for a reader with a certain set of expectations and assumptions, while also being short and easy to read. (and in this sense represent an ideal context for the question here about the iterplay between content and audience --5Dor essentially describes them as highly condensed sites of negotiation between content and audience, between what's being said who (it's assumed) is reading it.)
more concretely, in the content analysis tradition in sociology / media studies, long series of studies that have examined the ways that specific topis and issues are represented in hls.

meanwhile, in NLP, two very active lines of work on headlines -- first, a series of studies that treat hls as evidence about the future, in different ways -- in quantative finance, exploring correlations between hls and price movements in different kinds of investment vehicles; and, in the context of the journalism industry, predicting the "performance" of a headline -- CTR, shares, etc. meanwhile, another line of work on what might be thought of as "pathalogical" headlines -- eg, classifying if a hl is "clickbait" or "sensational," or identifying cases where the headline doesn't match the actual article content.
like these studies, we approach headlines from a standpoint of prediction -- in our case, predicting the *origin* of the headline. but, instead of trying to solve an applied engineering / product problem, we use the supervised learning problem as a descriptive and interpretive tool, not a solution to an engineering problem -- a way to measure the similarities between headlines, and a way to induce representations of the content of headlines that make it possible to "read" the news ecosystem at a scale that would otherwise be impossible.
...
% \includegraphics{figures/ts-ab-radial.png}
...
% \includegraphics{figures/ts-ab-radial-magnitude.png}
...
% \includegraphics{figures/bloomberg-fox-corr.png}
...
% % \includegraphics{figures/breitbart-fox-corr.png}
%
% Both of these involve roughly monotonic trends, but in other cases this also holds in the context of up-and-down fluctuations. For example, for Bloomberg and The Hill:
%
% % \includegraphics{figures/bloomberg-thehill-corr.png}
%
% When headlines are similar, they're less likely to be shared by the same users; and vice versa. Meanwhile, in other cases, the direction of the correlation itself seems to change over time. For example, HuffPo / BuzzFeed seems to be an interesting case -- for the majority of the data window, content and audience seem to be negatively correlated -- the headlines were drifting apart, but audience was genrally increasing, though inconsistently. But then, at the end, when the headline similarity dramatically falls off (this is the BuzzFeed "quiz" explosion), the audience correlation also falls off:
%
% % \includegraphics{figures/buzzfeed-huffingtonpost-corr.png}
...
% \includegraphics{figures/ts-ca-corrs.png}

And, zooming in on a larger set of the strongest pairs in each direction:

% \includegraphics{figures/ts-ca-pos-corrs.png}

% \includegraphics{figures/ts-ca-neg-corrs.png}
