...
Where, clearly, the overlap is largely driven by a particular \textit{style} of political reporting -- a kind of professional, inside-the-beltway view on politics; politics covered as a sport, a game that plays out across a stream of interactions, quarrels, gambits, moves, calculations. Specifically, the model is picking up on the use of a very distinctive set of abbreviations used by all three outlets -- namely \texttt{dems}, \texttt{gop}, \texttt{rep}.

which in a sense is just a orthographic decision; but also arguably some meaning to it -- positions reporting outside of either, a kind of third-party observer, both inside and outside of the game.

likewise with MSNBC

meanwhile, though the hill has highest audience overlaps with left-leaning outlets, it's also the one non-far-right outlet that makes inroads with the audiences of the far-right outlets -- with DC/BB, Hill is the one outlet outside of the Fox/DC/BB trio that cracks the top spots for audience similarity
...
though, also not just these orthographic ticks that the model is getting -- eg, compare to the overlap hls between DC/BB, which show a very strong focus on immigration and terrorism:

which is almost entirely missing with the overlaps with The Hill, which seem center fairly narrowly on politics per se -- trump administrator, capitol hill, congressional races.

interesting to compare this with the relationship between DC/BB and HuffPo. HuffPo also among the more misaligned outlets, though the pattern is quite different -- low correlation is driven by very low audience overlap with the three most right-leaning outlets (Fox, BB, and DC), though the
...
question of shared viewpoint is more complex -- does the hill/(dc/bb) overlap




The similarity here, in other words, appears to be driven by an overlap of attention and style -- the topics, people, and issues being covered, as well as the low-level linguistic style of the coverage -- the slang, the lexicon, the way of speaking, the journalistic "dialect." But, not so much by an overlap in \textit{viewpoint} -- in many cases, the headlines from The Hill are clearly coming from a more left-leaning (or, at least, less right-leaning) perspective.
