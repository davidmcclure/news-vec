...
How different, in a very precise sense, is a piece of content from Fox vs NYT? or, NYT vs WSJ, WSJ vs WaPo, WaPo vs Breitbart? most people have broad intuitions about this, but, if we measure it empirically -- what's

to what degree do different news outlets focus on different issues, "speak" in different ways?
this question of the separability of news content, distance between sources, cuts to heart of recent concerns of processes of fragmentation / polarization in media ecosystem, which tend to invovle an interplay of audience graph and content graph
most studies start with audience-level models, and then either define content in terms of audience, or study content in an unsupervised, exploratory context. in many cases, broad assumption that content and audience are essentially one-in-the same
but, how true is this actually? how linked are content and audience? when one changes, does the other follow suit? where, if ever, does this break down?

thesis surveys the news ecosystem from a standpoint of textual distance, focusing on a corpus of ~1.2M unique article headlines extracted from 70M links to articles from 15 major US outlets.
formualte the question as a supervised learning problem, in which classifiers are trained to predict the origin of a headline, the news outlet that produced it. this training objective makes it possible to induce high-quality distributed representations of hls, and also makes it possible to very precisely quantify the degree to which different outlets produce similar and dissimilar content.


with precise content graph in hand, can then


this "content graph" can then be put into conversation with other quantitative views of news landscape and identify places where outlets are speaking in ways that run counter to expectations, given the audiences who are consuming the content and the way that the outlet has sounded in the past.

first, systematically the degree to which similarities at the level of content correlate with similarities at the level of audiences, with specific focus on places where this correlation breaks down, where outlets serve as content bridges / mediators that expose audiences to content that might otherwise be partitioned off. find that content and audience overall tend to be aligned, but with some significant exceptions -- namely AP and hill are the two most misaligned outlets, and can perhaps look to (specific portions) their content as a kind of bottom-up signal for the types of topics / issues that might be effective at permeating across axes of political / cultural difference.

with these linked representations of content and audience graphs, then unroll over time and study as historically evolving systems. first, with headlines -- how stable are the linguistic profiles of major news organizations? study historical changes in headline similarity, degree to which outlets have moved into new configurations over time. buzzfeed -> quizzes; huffpo -> politics; DK (somewhat) away from politics; Fox -> tabloid.

finally, explore the degree to which these historical changes in content correlate with historical changes in audience composition, which provides a point of entry onto a larger set of questions about the causal interactions between the two systems -- as content becomes more similar, do audiences also converge, and vice versa? Overall, positive correlation between changes in content / audience, though the relationships for individual pairs of outlets are interestingly varied -- for fox and bloomberg, very strong positive; for fox and BB, very strong negative correlation -- headlines from Fox and BB have become less similar, but audience correlation has increased.
