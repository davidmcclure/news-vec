...
beyond these two most dramatic misalignments, a number of other interesting patterns with other outlets that at times seem to suggest a kind of taxonomy for the content-audience fit, diffrent ways that an outlet can occupy these graphs. eg, huffpo notable for extremely low audience correlations with Fox/DC/BB, even though the content distances are right around average; suggesting a very strong ideological aversion between HuffPo and these outlets, even though the headline distance is par for the course. WaPo is notable for having very high audience correlations with a number of other left-leaning outlets (NYT, MSBC, The Hill), even though the headline similarities are relatively low; suggesting Wapo represents a voice that is both consistently different from other left-leaning outlets but also consistently interesting to other left-leaning outlets. (perhaps a function of WaPo's unusual double-status as national political newspaper and DC local?) last but not least -- the three farthest-right outlets, Fox/BB/DC, all have very similar profiles -- very strong audience correlations with each other, low audience overlap with everyone else; but a much more continuous distribution over headline similarities, with AP and The Hill creeping unexpectedly high for Fox and DC respectively.

\section{Headlines as a historical process}

By measuring the \textit{distinguishability} of headlines, then, we were able to construct a precise representation of the relative similarities and differences among media outlets. Then, we compared these content-level similarities to similarities at the level of audience, which makes it possible to identify sites of misalignment between these graphs -- Fox distributes large quantites of "AP-style" content that otherwise doesn't fit into Fox's linguistic niche; The Hill occupies an unusual middle-ground in the landscape of political reporting, producing headlines that the classifiers alternatively mistake for both the center-left MSNBC and the far-right Breitbart and Daily Caller.

So far, though, we've been treating the headlines as a completely synchronic slice of data -- we're rolling up

In a sense, though, this represents a fairly radical

makes sense in the context of trying to assemble a kind of holistic model of different outlets since, say, the beginning of the Trump presidency. but it also suggests that there could be some very interesting information that's getting thrown out when we average
