...
as for twitter -- downside of course is that twitter, in very well-documented ways, isn't representative of the full media ecosystem. but, the advantage is that we can explore these questions with a level of scale and, perhaps even more important, consistency that would otherwise be impossible. twitter prevents us from making "wide" claims about the media ecosystem -- we can at most make platform-studies claims about what happens on twitter. but,

depth and granularity that would otherwise by difficult.

as for headlines -- downside is that we don't get a view of the actualy underlying article content, and, relationship between hls and article bodies is somewhat unclear
but -- headlines are also highly complex and rich textual objects -- a difficult-to-pin-down mix of summary, advertisement, metonymy, provocation; and, beyond the linguistic and (and even para-literary) interest of hls, there are a number of more intellectually pragmatic advantages to focusing on hls in this context
...
second, far from being a kind of afterthought, way in which headlines just are the content of the news, in a constituitive sense -- ~60\% of readers only read hls.
