...
How different, in a very precise sense, is an article from Fox vs NYT? or, NYT vs WSJ, WSJ vs WaPo, WaPo vs Breitbart? to what degree do different news outlets focus on different issues, "speak" in different ways?
this question of the separability of news content cuts to heart of recent studies of processes of fragmentation / polarization in media ecosystem, which tend to invovle an interplay of audience graph and content graph
most studies start with audience-level models, and then either define content in terms of audience, or study content in an unsupervised, exploratory context. in many cases, broad assumption that content and audience are essentiallly one-in-the same
but, how true is this actually? what's the exact relationship between content -- what's being said -- and audience -- who's reading it? how has this evolved over time? and, maybe most interseting -- where does it break down?

thesis surveys the news ecosystem from a standpoint of textual distance, focusing on a corpus of ~1.2M unique article headlines extracted from 70M links to news articles from 15 major US outlets.
supervised setting, classifiers that predict outlet given headline, which makes it possible to induce high-fidelity representations of hls, and also makes it possible to very precisely quantify the degree to which different outlets produce similar and dissimilar content.
