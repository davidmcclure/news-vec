...
All of which, we can see, have a distinctly AP flavor to them, though the content appears to focus on particular areas. Namely -- foreign affairs ("German police," "Ukranian rebels"), crime and terrorism ("Walmart shooting," "deadly Sinai attack"), and US news that's anchored by references to particular states ("Arkansas inmate," "Minnesota sheriff," "Oregon boy.") The closest pair, though, is literally a duplicate headline -- which, indeed is an AP story that's getting directly syndicated by Fox. Here, on foxnews.com, with the byline pointing back to AP:
...
But, just spot-checking a few other headlines, there are some here from Fox that aren't AP, that are getting produced in-house by Fox. For example, "Man reunited with baby he saved with CPR on side of highway" was written by Alexandria Hein for Fox's health section:
...
What's the overall ratio here? How many of the \texttt{foxnews.com} stories that show up close to AP in the embedding space are in-house Fox content, as opposed to AP syndications? This is a key question, in that it fundamentally affects what we might think of as the "direction" of the content bridge formed by AP. If the Fox/AP similarity is driven by Fox literally re-printing AP stories -- which otherwise are more commonly seen by the left-leaning audiences of NYT and WaPo -- then the presence of AP in the Fox portfolio constitues a left->right content bridge; but, if Fox is genuine

What's the ratio here of in-house Fox vs AP-syndicated content, among the headlines that are getting linked back to \texttt{foxnews.com}? This is a key question, in the sense that the answer significantly affects the interpretation of AP as a content "bridge." If Fox is producing most of these stories, then
